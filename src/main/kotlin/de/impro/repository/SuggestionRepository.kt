...
interface SuggestionRepository : CrudRepository<Suggestion, BigInteger> {
  ...
  @Query(
      " select new de.impro.dto.SuggestionGroupedDto(s.word, count(s.word)) from Suggestion as s where s.question = :question group by s.word")
  @io.micronaut.data.annotation.repeatable.QueryHints(
      value = [QueryHint(name = QueryHints.HINT_CACHEABLE, value = "true")])
  @Cacheable
